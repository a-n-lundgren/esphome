...
double TinyGPSPlus::distanceBetween(double lat1, double long1, double lat2, double long2)
{
  ...
  double delta = toRadians * (long1-long2);// radians(long1-long2);
  double sdlong = sin(delta);
  double cdlong = cos(delta);
  lat1 = toRadians * lat1; // radians(lat1);
  lat2 = toRadians * lat2; // radians(lat2);
  double slat1 = sin(lat1);
  double clat1 = cos(lat1);
  double slat2 = sin(lat2);
  double clat2 = cos(lat2);
  delta = (clat1 * slat2) - (slat1 * clat2 * cdlong);
  delta = delta * delta; //sq(delta);
  delta += (clat2 * sdlong) * (clat2 * sdlong); // sq(clat2 * sdlong);
  delta = sqrt(delta);
...
double TinyGPSPlus::courseTo(double lat1, double long1, double lat2, double long2)
{
  ...
  double dlon = toRadians * (long2-long1); // radians(long2-long1);
  lat1 = toRadians * lat1; // radians(lat1);
  lat2 = toRadians * lat2; // radians(lat2);
  ...
  return toDegrees * a2; // degrees(a2);
}
